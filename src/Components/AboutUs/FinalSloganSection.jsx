...
const FinalSloganSection = () => {
  ...
  // Reduced scale for mobile to prevent text cut-off
  const scale = useTransform(scrollYProgress, [0, 0.6, 1], [0.8, 1.5, 0]);
  ...
  const y = useTransform(scrollYProgress, [0, 0.5, 1], [20, -10, -50]);

  return (
    <section
      ref={ref}
      className="relative h-screen flex items-center justify-center bg-gradient-to-br from-gray-50 via-white to-gray-100 overflow-hidden px-2"
    >
      ...
      <motion.div
        className="relative z-10 text-center w-full max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8"
        style={{
          scale,
          opacity,
          y,
      ...
      >
        <motion.p
          ...
          viewport={{ once: true }}
          className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl xl:text-6xl font-bold mb-2 sm:mb-3 md:mb-4 text-gray-900 leading-tight"
        >
          Elevate Your Vibe,
        </motion.p>
        <motion.p
          ...
          className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl xl:text-6xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-cyan-600 to-purple-600 leading-tight"
        >
          CP Tech Yourself.
        </motion.p>
      </motion.div>
